...
var gruntOverride = require('./override');
...
GruntO.prototype = {
	...
	config: function (config) {
		if (_.isPlainObject(config)) {
			...
			_.each(config, function (v, k) {
				conf[k] = conf.hasOwnProperty(k) ? _.extend(conf[k], v) : v;
			});
		} else if (config != null) {
	...
	_run: function (func) {
		...
		gruntOModules.forEach(function (f) {
			...
			var context = new GruntOModuleContext(that.grunt, aliases, moduleTaskRefs, that._config, f.prefix, that._options);
		...
		this._statistic(aliases, moduleTaskRefs, gruntOModules, loadTime, time);
